...
if __name__ == '__main__':
    ...
    for title in categoryDict:
        ...
        if title in relationDict:
            ...
            for todo_data in todos:
                if title == todo_data["category"]:
                    parentCat[parentTitle].append({"1":[{todo_data["title"]:[]}]})
                    #parentCat[parentTitle].append({todo_data["title"]:[]}) #Test
            #print(" ", parentCat)
    ...
    print("\n\n3:")
    print("\ntree:", tree)
    print_tree(tree, 0, "📁", "🗄", todos,imgmath=0)
